...
export const addMarker = (map, locations) => {
    locations.forEach((loc) => {
        new mapboxgl.Marker({ color: '#880808', scale: '1' }).setLngLat([loc.lng, loc.lat]).addTo(map);
    })
}
...
export const customMarkers = (map, locations) => {
    ...
    locations.forEach((loc) => {
        ...
        marker = new mapboxgl.Marker({ element: el, draggable: true }).setLngLat([loc.lng, loc.lat]).addTo(map);
    })
    return marker;
}
...
export const clickMarker = (map, locations) => {
    ...
    marker = new mapboxgl.Marker(el).setLngLat([70.24066469880967, 31.989800013292694]).addTo(map);
